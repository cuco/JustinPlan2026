...
export async function GET(){
 ...
 try{const result=await storage().prepare('SELECT key,value FROM progress WHERE user_id = ?').bind(user.userId).all<{key:string,value:number}>();
 return Response.json({records:Object.fromEntries(result.results.map(r=>[r.key,r.value])),today:todayKey()},{headers});
